...
TV_Tags::TV_Tags(QWidget *parent) : QTreeView(parent) {
    ...
    connect(this, &TV_Tags::doubleClicked, this, &TV_Tags::myDoubleClicked);
 ...
 */
void TV_Tags::mousePressEvent(QMouseEvent *e) {
    if (e->button() == Qt::RightButton) {
        ...
        connect(btn_info, &QPushButton::clicked, this, &TV_Tags::btn_info_clicked);

        // button add tag
        QPushButton *btn_addtag = new QPushButton("add");
        lay1->addWidget(btn_addtag);
        connect(btn_addtag, &QPushButton::clicked, this, &TV_Tags::btn_addtag_clicked);

        // button find
        QPushButton *btn_find = new QPushButton("find");
        lay1->addWidget(btn_find);
        connect(btn_find, &QPushButton::clicked, this, &TV_Tags::btn_find_clicked);

        // button add sibling
        QPushButton *btn_add_sibling = new QPushButton("add sibling");
        lay1->addWidget(btn_add_sibling);
        connect(btn_add_sibling, &QPushButton::clicked, this, &TV_Tags::btn_add_sibling_clicked);

        // button add child
        QPushButton *btn_add_child = new QPushButton("add child");
        lay1->addWidget(btn_add_child);
        connect(btn_add_child, &QPushButton::clicked, this, &TV_Tags::btn_add_child_clicked);
        ...
        //        connect(btn_addtagchd, &QPushButton::clicked, this, &TV_Tags::btn_addtagchd_clicked);
        ...
        connect(btn_rentag, &QPushButton::clicked, this, &TV_Tags::btn_rentag_clicked);

        // button del tag
        QPushButton *btn_deltag = new QPushButton("del tag");
        lay1->addWidget(btn_deltag);
        connect(btn_deltag, &QPushButton::clicked, this, &TV_Tags::btn_deltag_clicked);

        // button paste
        QPushButton *btn_paste = new QPushButton("btn paste");
        lay1->addWidget(btn_paste);
        connect(btn_paste, &QPushButton::clicked, this, &TV_Tags::btn_paste_clicked);

        // button make inf
        QPushButton *btn_makeinf = new QPushButton("make inf");
        lay1->addWidget(btn_makeinf);
        connect(btn_makeinf, &QPushButton::clicked, this, &TV_Tags::btn_makeinf_clicked);
        ...
        connect(btn_makeinfofmarkedtag, &QPushButton::clicked, this, &TV_Tags::btn_makeinfofmarkedtag_clicked);

        // button make child
        QPushButton *btn_makechild = new QPushButton("make child");
        lay1->addWidget(btn_makechild);
        connect(btn_makechild, &QPushButton::clicked, this, &TV_Tags::btn_makechild_clicked);
...
void TV_Tags::startDrag(Qt::DropActions /*supportedActions*/) {
...
void TV_Tags::dragEnterEvent(QDragEnterEvent *event) {
    //    std::cerr << "debug - line: " << __LINE__ << " - file: " << __FILE__ << std::endl;
...
void TV_Tags::dragMoveEvent(QDragMoveEvent *event) {
    event->acceptProposedAction();
    ////    std::cerr << "debug - line: " << __LINE__ << " - file: " << __FILE__ << std::endl;
    //    setDropIndicatorShown(true);
 ...
 * Drop treeview item
 */
void TV_Tags::dropEvent(QDropEvent *event) {
 ...
 * Test
 */
void TV_Tags::btn_test_clicked(void) {
    dia_tags->close();
...
void TV_Tags::refresh_tree(void) {
...
void TV_Tags::myClicked(const QModelIndex &index) {
    //
    // (Click on tag)
    // ... do nothing ...
    //
...
void TV_Tags::show_imgs_from_tagid(int tagid) {
    //    std::cerr << "debug - line: " << __LINE__ << " - file: " << __FILE__ << std::endl;
 ...
 * Load img panel
 */
void TV_Tags::myDoubleClicked(const QModelIndex &index) {
...
//void TV_Tags::selectionChanged(const QItemSelection &selected, const QItemSelection &deselected) {
//    //
 ...
 * Choice tag parent
 */
void TV_Tags::btn_par_choice_clicked(void) {
...
void TV_Tags::btn_mark_clicked(void) {
    //
...
void TV_Tags::btn_info_clicked(void) {
    dia_tags->close();
...
void TV_Tags::btn_addtag_clicked(void) {
    dia_tags->close();
...
//void TV_Tags::tagfind_test(void) {
//    std::cerr << "debug - line: " << __LINE__ << " - file: " << __FILE__ << std::endl;
//}
 ...
 * Show prev found tag
 */
void TV_Tags::show_prev_found_tag(void) {
    if (qfs->show_index > 0) {
        qfs->show_index--;
    }
 ...
 * Show next found tag
 */
void TV_Tags::show_next_found_tag(void) {
 ...
 * Found all sub tags
 */
void TV_Tags::find_all_sub_tags(std::string tag_find, QModelIndex qmi_par) {
 ...
 * Found all tags
 */
void TV_Tags::find_all_tags(std::string tag_find) {
    qfs->qmis.clear();
    QStandardItem *rootitem = md->invisibleRootItem();
    // loop through all treeview items
    int col = 0;
 ...
 * Show found tag n
 */
void TV_Tags::show_found_tag_n(int n) {
    // check if n is valid
    if (n < 0) {
        return;
    }
 ...
 * Find
 */
void TV_Tags::btn_find_clicked(void) {
    dia_tags->close();
    //    DiaEditLine *dia_editline = new DiaEditLine(this);
    std::unique_ptr<DiaEditLine> dia_editline = std::make_unique<DiaEditLine>(this);
...
//        tag_find = str2;
        show_found_tag_n(0);
        connect(dia_tagfind, &DiaTagFind::diatagprev, this, &TV_Tags::show_prev_found_tag);
        connect(dia_tagfind, &DiaTagFind::diatagnext, this, &TV_Tags::show_next_found_tag);
//        dia_tagfind->exec();
 ...
 * Get selected tags
 */
std::vector<Tag> TV_Tags::get_selected_tags(void) {
    std::vector<Tag> tags;
    QModelIndexList indexes = selectionModel()->selectedIndexes();
 ...
 * Get parent tree tag
 */
Tag TV_Tags::get_parent_tree_tag(QModelIndex qmi_cur) {
 ...
 * Add sibling tag
 */
void TV_Tags::btn_add_sibling_clicked(void) {
 ...
 */
Tag TV_Tags::get_tag_from_QModelIndex(QModelIndex qmi) {
 ...
 * Add child tag
 */
void TV_Tags::btn_add_child_clicked(void) {
 ...
 */
void TV_Tags::btn_addtagchd_clicked(void) {
    dia_tags->close();
 ...
 * Rename Ttag
 */
void TV_Tags::btn_rentag_clicked(void) {
    dia_tags->close();
 ...
 */
void TV_Tags::btn_deltag_clicked(void) {
...
void TV_Tags::chk_and_make_inf(Tag par, Tag chd) {
    try {
...
void TV_Tags::chk_and_make_inf(Tag par, std::vector<Tag> chds) {
 ...
 */
void TV_Tags::btn_load_dir_files(void) {
 ...
 */
void TV_Tags::btn_load_files(void) {
...
void TV_Tags::btn_makeinf_clicked(void) {
    dia_tags->close();
...
// * Copy Tag
// */
//void TV_Tags::copy(void) {

//}



//Tag TV_Tags::get_marked_tag(void) {
...
//void TV_Tags::add_new_parent(Tag marked, Tag par) {
 ...
 */
void TV_Tags::btn_paste_clicked(void) {
 ...
 */
void TV_Tags::btn_makeinfofmarkedtag_clicked(void) {
    dia_tags->close();
 ...
 */
void TV_Tags::btn_makechild_clicked(void) {
    dia_tags->close();
    std::vector<Tag> chds = get_selected_tags();
    if (chds.size() != 1) {
        return;
 ...
 * (marked tag is showed on status line)
 */
void TV_Tags::btn_makepadofmarkedtag_clicked(void) {
...
void TV_Tags::btn_makeroot_clicked(void) {
    //
 ...
 * Tag search
 */
void TV_Tags::btn_search_clicked(void) {
    dia_tags->close();
    DiaMsgOkCancel *dia_msg_ok_cancel = new DiaMsgOkCancel(this);
 ...
 */
void TV_Tags::btn_removefrompar_clicked(void) {
    dia_tags->close();
 ...
 */
void TV_Tags::remove_items_from_par(QModelIndexList chds, QModelIndex par) {
...
void TV_Tags::add_subtree(std::vector<Tag> tags, Tag tag, QStandardItem *item, int level) {
    if (level > MAX_LEVEL) {
        return;
    }
    // add treeview tag
...
void TV_Tags::add_tags(std::vector<Tag> tags) {
    try {
        for (Tag tag : tags) {
...
int TV_Tags::get_seltags_count(void) {
    std::vector<int> ids;
...
std::vector<int> TV_Tags::get_selected_ids(void) {
    std::vector<int> ids;
